...
def lookup_cookies_batch(cookie_names: List[str]) -> Dict:
    ...
    for name in iterator:
        ...
        if name in cookie_db and cookie_db[name].get('category') == 'unknown':
            ...
            results[name] = cookie_db[name]
            continue
            
        try:
            info = lookup_cookie_with_page(name, page)
            results[name] = info
            cookie_db[name] = info
...
def lookup_cookie_with_page(name: str, page) -> Dict:
    ...
    try:
        ...
        # Add new function to get script URL
        def get_script_url() -> str:
            try:
                elements = page.locator('text=URL').all()
                for elem in elements:
                    parent_text = elem.locator('..').inner_text()
                    if ':' in parent_text:
                        value = parent_text.split(':', 1)[1].strip()
...
def analyze_cookie(name: str) -> Optional[Dict]:
    """Analyze a single cookie, now using batch lookup for efficiency"""
    # Check local database first
    if name in cookie_db:
        return cookie_db[name]
    
    # Look up single cookie using batch function
    results = lookup_cookies_batch([name])
...
# Register cleanup functions for exit
atexit.register(cleanup_browser)
atexit.register(save_cookie_database)

def test_amazon_cookies():
    """Modified test function to use batch lookup"""
    import json
    import os
    ...
    # Use batch lookup instead of individual lookups
    results = lookup_cookies_batch(list(cookies))
