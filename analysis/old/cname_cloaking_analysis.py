...
def get_outliers(group_data):
    q1 = group_data['potential_cname_cloaking'].quantile(0.25)
    q3 = group_data['potential_cname_cloaking'].quantile(0.75)
    iqr = q3 - q1
    upper_bound = q3 + 1.5 * iqr
    return group_data[group_data['potential_cname_cloaking'] > upper_bound]

# Create the box plot
plt.figure(figsize=(16, 8))

# Create box plot with black and white style
sns.boxplot(data=df_loaded, x='profile', y='potential_cname_cloaking', 
            order=all_profiles,
...
for i, profile in enumerate(all_profiles):
    ...
    if not outliers.empty:
        # Add profile index and sort by count
...
if all_outliers:
    ...
    top_outliers = all_outliers_df.nlargest(12, 'potential_cname_cloaking')

    # Annotate top outliers
    for _, outlier in top_outliers.iterrows():
        plt.annotate(
            outlier['domain'],
            xy=(outlier['profile_idx'], outlier['potential_cname_cloaking']),
            xytext=(10, 10),
...
y_max = df_loaded['potential_cname_cloaking'].max()
current_position = 0
...
# Customize the plot
plt.title('Distribution of Potential CNAME Cloaking per Profile\n(For domains that loaded successfully across all profiles)',
          fontsize=16, pad=40)
plt.ylabel('Number of Potential CNAME Cloaking Instances', fontsize=14, labelpad=10)
...
plt.savefig('potential_cname_cloaking_distribution.png', dpi=300, bbox_inches='tight')
plt.show()

# Print summary statistics
print("\nSummary Statistics:")
for profile in all_profiles:
    profile_data = df_loaded[df_loaded['profile'] == profile]['potential_cname_cloaking']
...
print("\nOutliers (domains with unusually high CNAME cloaking instances):")
for profile in all_profiles:
    profile_data = df_loaded[df_loaded['profile'] == profile]
    outliers = get_outliers(profile_data)
    ...
    if not outliers.empty:
        print(f"\n{DISPLAY_NAMES.get(profile, profile)}:")
        for _, row in outliers.sort_values('potential_cname_cloaking', ascending=False).iterrows():
