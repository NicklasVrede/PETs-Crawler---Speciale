...
def plot_all_requests(profile_data):
    """Create a stacked bar chart of all requests by category."""
    ...
    # Calculate the total requests for baseline profile to use as baseline (100%)
    ...
    plt.savefig('analysis/graphs/all_requests_by_category.png',
                dpi=300, bbox_inches='tight')
    plt.show()
    
    # Print summary statistics
    print("\nSummary of All Requests by Profile:")
    for profile in ordered_profiles:
...
def main():
    ...
    for profile in profiles:
        category_counts = analyze_all_requests(profile, successful_domains)
        if category_counts:  # Only include profiles with data
            profile_data[profile] = category_counts
    
    # Create visualization
    if profile_data:
        plot_all_requests(profile_data)
    else:
        print("No requests found in any profile.")
