...
output_dir = os.path.join(project_root, 'analysis', 'graphs', 'cookie_similarities_by_category')
                ...
                if 'cookies' in data and '1' in data['cookies']:
                    for cookie in data['cookies']['1']:
                        if 'classification' in cookie:
                            # Use the category exactly as it appears
...
def calculate_cookie_similarity_by_category(cookies1, cookies2, category):
    ...
    if not set1 and not set2:
        return 1.0  # Both sets empty means they're identical (both have no cookies)
...
def analyze_banner_extension_similarities_by_category():
    ...
    # Define cookie categories as they appear in the data
    ...
    # Create a separate plot for each category (instead of groups)
    for category in cookie_categories:
        ...
        # Add text annotations with outline for better visibility
        for i in range(len(ordered_profiles)):
            for j in range(len(ordered_profiles)):
                ...
                text = ax.text(j, i, f'{value:.3f}',
                             ...
                             fontweight='bold',
                             path_effects=[
                                 Stroke(linewidth=2, foreground='white' if text_color == 'black' else 'black'),
                                 Normal()
                             ])
        ...
        # Save with padding
        filename = f'cookie_extension_similarity_{category.lower()}.png'
