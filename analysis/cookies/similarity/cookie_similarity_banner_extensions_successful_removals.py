...
def get_domains_with_banner_removals():
    """Get domains where banners were successfully removed across all consent management profiles."""
    # Read the CSV file
    df = pd.read_csv('data/csv/final_data2.csv')
    
    # Define consent management profiles
    consent_profiles = [
    ...
    # Get domains where banners were removed for each consent profile
    domains_by_profile = {}
    for profile in consent_profiles:
        domains = set(df[
            (df['profile'] == profile) & 
            (df['page_loaded'] == True) &
            (df['banner_removed'] == True)
        ]['domain'])
        domains_by_profile[profile] = domains
    
    # Find intersection of domains across all profiles and baseline
    all_domains = set.intersection(baseline_domains, *domains_by_profile.values())
    
    print(f"Found {len(all_domains)} domains that:")
    print(f"- Loaded successfully in baseline profile")
    print(f"- Loaded successfully AND had banners removed by all consent management extensions")
...
def analyze_banner_extension_similarities_by_category():
    ...
    # Get domains where banners were removed by all extensions
    successful_domains = get_domains_with_banner_removals()
